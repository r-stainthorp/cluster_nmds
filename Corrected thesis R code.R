...
clstructure = lapply(unique(clids$id), function(dend1){clids[clids$id == dend1,'cdr']}) 
#get memberships of cluster s
clstructure[[1]] 
clstructure[[2]]
clstructure[[3]]
clstructure[[4]]
...
vegMat<-(vegCover[2:1779,2:23])

# NOW USE THE specaccum FUNCTION TO GENERATE THE SPECIES ACCUMULATION CURVE INFORMATION:

spp.curve<-specaccum(comm=vegMat,method="random",permutations=1000)
plot(spp.curve,ylim=0,100)
